...
# (!) Выяснилось, что счет порций идет с 1, а в массиве элементы с нулевого.
# В итоге съедалась 08 размер и 00 - и этого никто не замечал целый месяц!
# Кароч надо -1
print DEBUG "Номер порции: ", $arg2, "\r\n\r\n";	
	
	# $p_sz = $u_sz[$portion_num];
	$p_sz = $u_sz[$portion_num-1];
	@u_sz = ();
	push(@u_sz, $p_sz);
