		...
		if ($('div#pult_wrap').hasClass('p_off')) {
		  	...
		  	$('div#pult').animate({'opacity':'0'}, {
		  		...
		  		complete:  function() 	{
					...
					// события для основной палитры
					if ( !($("div#dial_more").is(":visible")) ) {
						// $('.con2').append('видима ' + "<br/>");
		  	...
		  	$('div#pult_wrap').animate({width: 100, marginLeft: 0}, {
		  		duration: 800,
		  		complete:  function() 	{
...
// [fold~ NaC6]
			...
			// $('div#pult_outer').css('opacity','0'); 
			// $('div#pult_outer').css('margin-left','-40px'); 
			...
			$('div#pult_wrap').html("");
		  	...
		  	// $('div#pult').animate({'opacity':'1'}); // Показываем кнопку
		  	$('div#pult_wrap').addClass('p_off');
		  	$('div#pult_wrap').removeClass('p_on');

			$('div#p_col_block').unbind('click');
		// }
...
$('div#ic_more_but, div#pr_block_c img').bind('click', function() {
	...
	if (!($('div#dial_more').length)) { // Если элемента еще нет - создаем
				if ($('div#pult_wrap').hasClass('p_on')) {
			$('div#cheek_lower').trigger('click'); // Закрыть пульт
		}
		$('body').dial_more(); // Активация

		// Привязка обработчиков к элементам палитры диалога	
		$('div.pal_col').bind('click', dial_pal_click);

		// Выключение обработчиков иных элементов - пульта, палитры, щек
		// $('div#proto_up_button').bind
		// $('div#p_col_block').unbind('click');
		/*		
		$('div#div_ic_block_C').unbind('click');
		$('div#cheek_lower, div#cheek_upper').unbind('click');
		*/

	}
	else { // Если уже есть, нужно убрать
				// Убираем палитру
		$('div.fade2').remove(); $('div#dial_more').remove();

		$('.con2').append('Убираем палитру' + "<br/>");
		// Если пульт открыт, палитра убирается и нет уже привязки по основной палитре, 
		// то ее нужно организовать
		// (!) Что-то не работает...
		if ($('div#pult_wrap').hasClass('p_off')) {		
			
			// $(obj).off('click').on('click', function {});
			$('div#p_col_block').off('click').on('click', side_color_sel_click);
			// $('div#p_col_block').bind( 'click', side_color_sel_click);
		}

		// Восстанавливаем обработчик пульта
		// $('div#div_ic_block_C').bind('click', pult_click);
